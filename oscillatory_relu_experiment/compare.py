...
def train_model(X_train, y_train, X_test, y_test, activation_type='relu', lr=1e-3, epochs=20, batch_size=128, device='cpu'):
    model = MLP(40, [256, 256], 10, activation_type=activation_type).to(device)
...
def run_experiment():
    ...
    activation_types = ['relu', 'leaky_relu', 'snake', 'orelu']
    best_lrs = {}
    ...
    plt.tight_layout()
    plt.savefig("oscillatory_relu_experiment/comparison.png")

    # Visualize ORelu
    plt.figure(figsize=(8, 6))
    x_range = torch.linspace(-5, 5, 500)
    # Get one ORelu from the last trained model
    last_model = MLP(40, [256, 256], 10, activation_type='orelu').to('cpu')
    # Let's use parameters from a trained model if possible, or just some example parameters
    # Actually, let's plot a few learned ones from the first ORelu layer
    with torch.no_grad():
        # Get the first ORelu layer
        orelu_layer = None
        for layer in model.net:
            if isinstance(layer, ORelu):
                orelu_layer = layer
                break

        if orelu_layer:
            alpha = orelu_layer.alpha.cpu().numpy()
            beta = orelu_layer.beta.cpu().numpy()

            plt.plot(x_range.numpy(), torch.relu(x_range).numpy(), 'k--', label='Standard ReLU', alpha=0.5)
            for i in range(min(5, len(alpha))):
                y = torch.relu(x_range) + alpha[i] * torch.sin(beta[i] * x_range)
                plt.plot(x_range.numpy(), y.numpy(), label=f'Learned ORelu {i} (a={alpha[i]:.3f}, b={beta[i]:.2f})')
        else:
            plt.plot(x_range.numpy(), torch.relu(x_range).numpy(), label='ReLU')

    plt.title("Visualization of Learned ORelu Activations")
    plt.xlabel("Input")
    plt.ylabel("Output")
    plt.legend()
    plt.grid(True)
    plt.savefig("oscillatory_relu_experiment/learned_activations.png")

    # Summary results
    with open("oscillatory_relu_experiment/results.txt", "w") as f:
        for act in activation_types:
