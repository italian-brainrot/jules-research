...
from model import BaselineMLP, GLCMMLP, GLCMAugmentedMLP
...
def train_model(model, X_train, y_train, X_test, y_test, lr, epochs=30, batch_size=256):
...
def objective(trial, model_type, X_train, y_train, X_test, y_test):
    ...
    elif model_type == "glcm":
        model = GLCMMLP(input_dim=40, num_bins=num_bins, offsets=offsets, hidden_dim=hidden_dim, output_dim=10)
    elif model_type == "glcm_augmented":
        model = GLCMAugmentedMLP(input_dim=40, num_bins=num_bins, offsets=offsets, hidden_dim=hidden_dim, output_dim=10)
...
def main():
    ...
    results = {}
    exp_dir = "differentiable_glcm_experiment"

    for model_type in ["baseline", "glcm", "glcm_augmented"]:
    ...
    with open(results_path, "w") as f:
        for model_type, (mean, std) in final_results.items():
            f.write(f"{model_type}: {mean:.4f} +/- {std:.4f} (Best LR: {results[model_type]['best_lr']:.6f})\n")
    ...
    plt.ylabel('Accuracy')
    plt.title('Differentiable GLCM Experiment Results on MNIST-1D')
    plt.ylim(0, 1.0)
    plot_path = os.path.join(exp_dir, "results.png")
    plt.savefig(plot_path)
    print(f"Results saved to {results_path} and {plot_path}")
